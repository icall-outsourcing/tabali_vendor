...
@section('content')
    ...
    <div class="Append roles_list col-md-4">
        <div class="form-group">
          ...
          {!! Form::select("roles_list[]",$roles ,$EditData->RolesList,["class" => "form-control select2","id" => "role","required" =>"required",'multiple'=>'multiple',"style"=>"width:100%"])!!}
        </div>
    </div>
    <div class="Append permissions_list col-md-4">
        <div class="form-group">
          <label for="Permissions">{{ trans('form.permissions') }}</label>
          {!! Form::select("permissions_list[]",$permissions ,$EditData->PermissionsList,["class" => "form-control select2","id" => "permissions","required" =>"required",'multiple'=>'multiple',"style"=>"width:100%"])!!}
        </div>
    </div>
            ...
            @foreach($printers as $print)
              <div class="panel-body">
                <div class="row">                  
                  ...
                  @if(array_search($print->id, array_column($EditData->printers->toArray(), 'id')) !== false)                    
                      <div class="col-xs-2 text-center"> <input type="checkbox" name="printers[]" value="{{$print->id}}" checked></div>
                  @else 
