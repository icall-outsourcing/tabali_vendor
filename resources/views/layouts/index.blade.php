					...
					@endif
						@foreach($model->getCasts() as $key => $column)
							...
							@elseif (strpos($column, '_id') !== false) 
								...
								<td class="col-{{$column}}">
									...
									@if($column == 'order_id'){{$row->$column}}@endif
								</td>
						...
						<td class="text-center col-action col-md-1">
							...
							@if(Auth::user()->is('admin'))
								@if($model->model_name == 'User')
									<button data-route="{{URL::route($model->model_name.'.destroy',$row->id)}}" id="{{$row->id}}" data-token="{{ csrf_token() }}" type="button" class="destroy btn btn-danger btn-xs"><i class="fa fa-trash-o"></i></button> 
								@endif
							@endif
