			...
			<?php foreach($myModal->getCasts() as $key => $column): ?>
				...
				<?php elseif(strpos($column, '_id') !== false): ?> 
					...
					<td class="col-<?php echo e($column); ?>">
						<?php if($column == 'order_id'): ?><?php echo e($row->orderid); ?><?php endif; ?>
			...
			<?php if(!empty($myModal->model_name)): ?>
				<td class="text-center col-action">
					<a 			  href="#" id="showorder" data-route="<?php echo e(URL::route($myModal->model_name.'.show',   $row->id)); ?>" class="Edit btn btn-info btn-xs" ><i class=" fa fa-eye"></i></a>
			   		...
			   		<?php if($myModal->model_name == 'User'): ?>
			   		    <button data-route="<?php echo e(URL::route($myModal->model_name.'.destroy',$row->id)); ?>" id="<?php echo e($row->id); ?>" data-token="<?php echo e(csrf_token()); ?>" type="button" class="destroy btn btn-danger btn-xs"><i class="fa fa-trash-o"></i></button> 
			   		<?php endif; ?>
			    </td>
		    <?php endif; ?>
		</tr>
