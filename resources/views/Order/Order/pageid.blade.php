	...
	@foreach($DataTable as $row )
		...
		@if (!empty($myModal->model_name))
			...
			@foreach($myModal->getCasts() as $key => $column)
                ...
                @elseif ($column === 'total')
						    <td class="col-{{$column}}" id="{{$row->$column}}">							
								@php $discount = ( 100 - $row->discount) / 100;  $Afterdiscount =  number_format($row->total *  $discount,2) @endphp
								{{number_format((float)$Afterdiscount + $row->deliveryfees + $row->taxfees -($row->voucher_amount), 2, '.', '')}}								
							</td>
